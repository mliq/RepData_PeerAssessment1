...
```{r fig.height=4}
plot(cdata$date,cdata$steps.sum,type="h",ylab="Total Steps per Day",xlab="Day")
```
...
mean2 <- function (x) { median(x,na.rm="TRUE") } 
## Summarize data, then plot.
cdata2 <- summaryBy(steps ~ interval, data=data, FUN=c(mean2))
plot(cdata2$interval,cdata2$steps.mean2,type="l",
  ylab="Average Steps Taken",xlab="5-Minute Interval")
```
...
plot(ncdata$date,ncdata$steps.sum,type="h",
  ylab="Total Steps per Day",xlab="Day")
...
xyplot(steps.mean2~interval | days, data=daysdata, type="l",layout=c(1,2),xlab="Interval",ylab="Number of steps")
```
